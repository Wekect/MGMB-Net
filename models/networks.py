...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False,
    ...
    Parameters:
        ...
        netG (str) -- the architecture's name: msmbnet
    ...
    if netG == 'msmbnet':
        net = msmbnet(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, use_attention=True)
    else:
...
def styprcloss(gpu_ids=[]):
    net = VGG16FeatureExtrator()
    if len(gpu_ids)>0:
...
class mask_guide2(nn.Module):
    def __init__(self, channel):
        super(mask_guide2, self).__init__()
        self.maskcov1 = torch.nn.Conv2d(channel, int(channel/2),4,2,1)
        self.maskcov2 = torch.nn.Conv2d(int(channel/2), channel,4,2,1)
        self.maskcov3 = torch.nn.Conv2d(channel, channel*2,4,2,1)
        self.maskcov4 = torch.nn.Conv2d(channel*2, channel,4,2,1)
        self.maskcov5 = torch.nn.Conv2d(channel*2, int(channel/2),4,2,1)
        self.maskcov6 = torch.nn.Conv2d(channel, 4,4,2,1)


        self.sigmoidmask = nn.Sigmoid()
        self.ru = nn.ReLU()
        self.mergecov1 = torch.nn.Conv2d(6,32,1,1,0)
        self.mergecov2 = torch.nn.Conv2d(32,64,1,1,0)
        self.mergecov3 = torch.nn.Conv2d(64,32,1,1,0)
        self.mergecov4 = torch.nn.Conv2d(32,1,1,1,0)
        self.bn1 = nn.BatchNorm2d(32)
        self.bn2 = nn.BatchNorm2d(32)

        self.mergecov01 = torch.nn.Conv2d(5,32,1,1,0)
        self.mergecov02 = torch.nn.Conv2d(32,64,1,1,0)
        self.mergecov03 = torch.nn.Conv2d(64,32,1,1,0)
        self.mergecov04 = torch.nn.Conv2d(32,1,1,1,0)
    
    def forward(self, feature, mask, premask = None):
        if premask is not None and mask is not None:
            featuremask0 = self.maskcov1(feature)
            featuremask1 = self.maskcov2(featuremask0)
            featuremask2 = self.maskcov3(featuremask1)

            featuremask2d = self.maskcov4(featuremask2)
            featuremask1d = torch.cat([featuremask2d, featuremask1], 1)
            featuremask0d = self.maskcov5(featuremask1d)
            featuremask0d = torch.cat([featuremask0d, featuremask0], 1)
            featuremask0d = self.maskcov6(featuremask0d)

            fmask = self.sigmoidmask(featuremask0d)
            maskmerge = torch.cat([fmask, mask, premask], 1)
            mergeb = self.mergecov1(maskmerge)
            merge = self.bn1(mergeb)
            merge = self.mergecov2(merge)
            merge = self.ru(merge)
            merge = self.mergecov3(merge)
            merge = self.bn2(merge)
            merge = merge + mergeb
            merge = self.mergecov4(merge)
            result = self.ru(merge)

            return result

        if premask is not None and mask is None:
            featuremask0 = self.maskcov1(feature)
            featuremask1 = self.maskcov2(featuremask0)
            featuremask2 = self.maskcov3(featuremask1)
            ...
            fmask = self.sigmoidmask(featuremask0d)
            maskmerge = torch.cat([fmask, premask], 1)
            mergeb = self.mergecov01(maskmerge)
            merge = self.bn1(mergeb)
            merge = self.mergecov02(merge)
            merge = self.ru(merge)
            merge = self.mergecov03(merge)
            merge = self.bn2(merge)
            merge = merge + mergeb
            merge = self.mergecov04(merge)
            result = self.ru(merge)

            return result         
        if premask is None and mask is not None:
            featuremask0 = self.maskcov1(feature)
            featuremask1 = self.maskcov2(featuremask0)
            featuremask2 = self.maskcov3(featuremask1)
            ...
            fmask = self.sigmoidmask(featuremask0d)
            maskmerge = torch.cat([fmask, mask], 1)
            mergeb = self.mergecov01(maskmerge)
            merge = self.bn1(mergeb)
            merge = self.mergecov02(merge)
            merge = self.ru(merge)
...
class msmbnet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=RAIN, 
        ...
        if use_attention:
            self.model_layer11att = nn.Sequential(nn.Conv2d(ngf*8+32, ngf*8+32, kernel_size=1, stride=1), nn.Sigmoid())

        self.model_layer12        = get_act_dconv(nn.ReLU(True), ngf*8+32, ngf*2, 4, 2, 1, False)
        self.model_layer12norm    = norm_type_list[norm_type_indicator[12]](ngf*2)
        if use_attention:
            self.model_layer12att = nn.Sequential(nn.Conv2d(ngf*4+16, ngf*4+16, kernel_size=1, stride=1), nn.Sigmoid())

        self.model_layer13        = get_act_dconv(nn.ReLU(True), ngf*4+16, ngf, 4, 2, 1, False)
        self.model_layer13norm    = norm_type_list[norm_type_indicator[13]](ngf)
        if use_attention:
            self.model_layer13att = nn.Sequential(nn.Conv2d(ngf*5+16, ngf*5+16, kernel_size=1, stride=1), nn.Sigmoid())
        self.model_out = nn.Sequential(nn.ReLU(True), nn.ConvTranspose2d(ngf * 5+16, output_nc, kernel_size=4, stride=2, padding=1), nn.Tanh())


        self.unet_layer1 = get_act_conv(nn.LeakyReLU(0.2, True), ngf*2, ngf*4, 4, 2, 1, False)
        self.unet_layer2 = get_act_conv(nn.LeakyReLU(0.2, True), ngf*4, ngf*8, 4, 2, 1, False)
        self.unet_layer3 = get_act_conv(nn.LeakyReLU(0.2, True), ngf*8, ngf*4, 4, 2, 1, False)
        self.unet_layer4 = get_act_conv(nn.LeakyReLU(0.2, True), ngf*8, ngf, 4, 2, 1, False)
        self.unet_norm2 = norm_type_list[norm_type_indicator[0]](ngf*8)

        self.maskguide_n2 = mask_guide2(256)
        self.maskguide_n1 = mask_guide2(128)
        self.maskguide_n0 = mask_guide2(256)
    ...
    def forward(self, x, mask):
        mask0 = F.interpolate(mask, scale_factor=1/2)
        mask1 = F.interpolate(mask, scale_factor=1/4)
        mask2 = F.interpolate(mask, scale_factor=1/8)

        xf = x * mask
        xb = x * (1-mask)
        x_all = self.model_layer0(x)
        xf0 = self.model_layer0(xf)
        xb0 = self.model_layer0(xb)

        x0 = torch.cat([xf0, xb0], 1)
        xu1 = self.unet_layer1(x0)
        xu2 = self.unet_layer2(xu1)
        xu3 = self.unet_layer3(xu2)
        xu3 = torch.cat([xu1, xu3], 1)
        xu3 = self.unet_norm2(xu3)
        xu4 = self.unet_layer4(xu3)
        x0 = torch.cat([x_all, xu4, x0], 1)
        ...
        maskg2 = self.maskguide_n2(x2, mask2, None)
        maskg2e = maskg2.expand(-1, int(x2.size()[1]/8),-1,-1)
        x2 = torch.cat([x2, maskg2e], 1)
        mask2up = F.interpolate(maskg2, scale_factor=2)
        mask2up = self.sig2(mask2up)

        maskg1 = self.maskguide_n1(x1, mask1, mask2up)
        maskg1e = maskg1.expand(-1, int(x1.size()[1]/8),-1,-1)
        x1 = torch.cat([x1, maskg1e], 1)
        mask1up = F.interpolate(maskg1, scale_factor=2)
        mask1up = self.sig1(mask1up)

        maskg0 = self.maskguide_n1(x0, None, mask1up)
        maskg0e = maskg0.expand(-1, int(x0.size()[1]/16),-1,-1)
        x0 = torch.cat([x0, maskg0e], 1)
        mask0up = F.interpolate(maskg0, scale_factor=2)
        mask0up = self.sig0(mask0up)
        ...
        return mask0up, mask1up, mask2up, out
    
    def processImage(self, x, mask):
        pred = self.forward(x, mask)

        boundary = mask.clone()
        for ith in range(len(mask[:,0,0,0])):
            boundary_n = mask[ith,:,:,:].cpu().float().numpy()
            boundary_n = np.transpose(boundary_n, (1,2,0))
            kernel_dil = cv2.getStructuringElement(cv2.MPRPH_RECT, (3,3))
            boundary_dil = cv2.dilate(boundary_n, kernel_dil)
            bound = torch.from_numpy(boundary_dil).unsqueeze(0).unsqueeze(0).cuda()
            boundary[ith,:,:,:] = bound

        return pred * boundary + x[:,:3,:,:] * (1 - boundary)
...
class VGG16FeatureExtrator(nn.module):
    def __init__(self):
        super().__init__()
        vgg16 = models.vgg16(pretrained=False)
        pre = torch.load('.../pretrain/vgg16-397923af.pth')
